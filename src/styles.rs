...
use iced::{color, Background, Border, Color, Length, Theme};
...
static COLORS: LazyLock<[ColorStyles; 3]> = LazyLock::new(|| {
    [
        create_color_style(
...
pub fn create_button(
    content: &str,
    msg: Message,
    style: impl Fn(&Theme, Status) -> button::Style + 'static,
) -> Button<'_, Message> {
...
fn button_style(background_color: Color) -> button::Style {
    button::Style {
        border: Border {
            radius: Radius::new((BUTTON_DIAMETER / 2) as f32),
            ..Default::default()
        },
...
pub fn number_button_style(_: &Theme, status: Status) -> button::Style {
    let cs = &COLORS[unsafe { STYLE_IDX }];
    match status {
        Status::Active => button_style(cs.num_button_active),
        Status::Hovered => button_style(cs.num_button_hover),
        Status::Pressed => button_style(cs.num_button_press),
        Status::Disabled => Default::default(),
    }
}

pub fn operator_button_style(_: &Theme, status: Status) -> button::Style {
    let cs = &COLORS[unsafe { STYLE_IDX }];
    match status {
        Status::Active => button_style(cs.ops_button_active),
        Status::Hovered => button_style(cs.ops_button_hover),
        Status::Pressed => button_style(cs.ops_button_press),
        Status::Disabled => Default::default(),
    }
}

pub fn enter_button_style(_: &Theme, status: Status) -> button::Style {
    let cs = &COLORS[unsafe { STYLE_IDX }];
    match status {
        Status::Active => button_style(cs.etr_button_active),
        Status::Hovered => button_style(cs.etr_button_hover),
        Status::Pressed => button_style(cs.etr_button_press),
        Status::Disabled => Default::default(),
    }
}

pub fn screen_container_style(_: &Theme) -> container::Style {
    container::Style {
...
pub fn calculator_background_style(_: &Theme) -> container::Style {
    container::Style {
